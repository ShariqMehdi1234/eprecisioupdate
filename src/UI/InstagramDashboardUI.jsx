...
import BreadCrumbInstagramUI from './BreadCrumbInstagramUI';
...
// const client = new W3CWebSocket('ws://52.56.232.213:9090/spotify-socket');
...
function InstagramDashboardUI() {
  ...
  useEffect(() => {
    ...
    axios.post('/previous-instagram-results?user=ehtisham')
  ...
  const ids = dataSoundCloud.map(o => o.scrapper_name)
  const PreviousListText = dataSoundCloud.filter(({scrapper_name}, index) => !ids.includes(scrapper_name, index + 1))
  ...
  const filteredItemsArrayRowsData = dataSoundCloud.filter(person => (filterTextSelect === '' ? person.scrapper_name: person.scrapper_name === filterTextSelect)).map((item,index) =>
        ...
        <div className='row container-fuild-margin'>
          <div className='col-lg-12 col-md-12 col-sm-12 container-padding'>
            <div className={'filter-div'}>
              <Form.Select className='filter-select' onChange={PreviousListFilterOnChange} style={{ pointerEvents: 'none', backgroundColor: '#efefef' }}>
                <option value={''}>Ed Sheeran...</option>
                {PreviousListText.map((item, index) => (
                    <option key={index} value={item.scrapper_name}>{item.scrapper_name}</option>
                ))}
              </Form.Select>
